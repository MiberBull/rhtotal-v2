...
import { MSG_DIALOG, EMAIL_LOANS, KEYS_STORAGE, EMAIL_SUPPORT, IMG_MAIL_HEADER, TEMPLATES_FINTECH } from '../../environments/environments';
...
@Component({
  selector: 'page-velo-cash',
  templateUrl: 'velo-cash.html',
})
export class VeloCashPage {
  ...
  tipoAnticipo: string = "Velocash"; // Tipo de Anticipo
  ...
  constructor(
    ...
    this.correspondingAmount = this.data.paymentProperties[0].correspondingAmount;
    this.workedDays = this.data.paymentProperties[0].workedDays;
    this.acomulatedAmount = this.data.paymentProperties[0].acomulatedAmount;
    this.wayToPay = Conversions.UpperFirstLetter(this.data.paymentProperties[0].formadePago);
    this.nextPayment = this.data.paymentProperties[0].nextAmount;
    this.nextDatePayment = this.dateFormat(this.data.paymentProperties[0].siguientePago);
    this.commission = this.data.paymentProperties[0].commission;
    this.requestedAmount = this.data.paymentProperties[0].commisiones.montoDeposito;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad Velocash');
    this.serviceValidator.checkServiceStatus();
  }
    ...
    let alert = this.alertCtrl.create({
      ...
      },
      {
        ...
        handler: () => {
          ...
          this.paysheet_provier.loanVelocash(this.data).subscribe( () => {
