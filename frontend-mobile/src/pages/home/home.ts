...
import { Platform, NavController, MenuController, Nav, Slides, AlertController } from 'ionic-angular';
...
export class HomePage {
  ...
  ionViewDidLoad(){
    ...
    let imagesStorage;
    if( imagesStorage = this.storage_provider.getItem( KEYS_STORAGE.IMAGES_BANNER )) {
      ...
      .subscribe( (resp:any[]) => {
        ...
        this.slidesArray = [];
        let defaultImage = ['assets/imgs/default.png'];
  ...
  blockService(){
    ...
    this.users.employeesByProject('INNOHUB').subscribe(data => {
      let list = data.employee;
      let isBlackList = list.toString().toLocaleLowerCase().trim().includes(this.email.toString().toLocaleLowerCase());
