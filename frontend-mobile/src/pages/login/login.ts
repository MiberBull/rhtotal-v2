...
export class LoginPage {
  ...
  onLogin(){
    ...
    if( this.validateEmail( this.loginForm.value.email ) ) {
      let userTO: UserTO = new UserTO();
      userTO = this.loginForm.getRawValue();
  ...
  blockUser(reposne:LoginTO){
    ...
    this._loginProvider.blockingUserByIntents(reposne)
  ...
  getImagesForUser() {
    ...
    .subscribe( (resp:any[]) => {
      if( resp.length > 0 ) {
        ...
        let images = ['assets/imgs/default.png'];
  ...
  getDataPerfil(id:number){
    this.user_provider.getInfoCredential(id).subscribe(data =>{
  ...
  onLoginReopen(){
    this.eventManager.setIsLoadingEvent(true);
    let userTO: UserTO = new UserTO(); // Revisar inicializacion
    userTO = this.loginForm.getRawValue();
    console.log(userTO);
    let loginTO: LoginTO = new LoginTO();
    loginTO.user = this.storage_provider.getItem( KEYS_STORAGE.USER );
    loginTO.user.email = this.storage_provider.getItem( KEYS_STORAGE.USER )['email'].toLocaleLowerCase();
    loginTO.flag=0;
    loginTO.block=false;
