...
export class HiringDataPage {
  ...
  geContratingDataRHTotal() {
    this.users.getContratingByIdUser(this.userObejct.id).subscribe(
      (data: ContratingDataTO) => {
  ...
  getSicoContratingData(curp: string, name: string) {
    ...
    this.users.getWorkInfoSico(curp, name).subscribe(
      (data) => {
        if (data && data.data && data.data.pago && data.data.pago.length > 0) {
          const contrect = new ContratingDataTO();
          contrect.qtSalary = data.data.pago[0].salarioRealPercibidoMes;
          contrect.job = (data.data.personal as any).puesto;
  ...
  getEployeName() {
    this.users.getEmployeeById(this.userObejct.id).subscribe(
      (data: EmployeeComplementaryTO) => {
        const nameCompouse: string = data.employee.name.trim().replace(' ', '+');
  ...
  getInfoForm() {
    ...
    const contract = new ContratingDataTO();
  ...
  saveContratingData(contratingData: ContratingDataTO) {
    this.loading.setIsLoadingEvent(true);
    this.users.saveOrUpdateEmployeeContrating(contratingData).subscribe(
      (data) => {
        if (data) {
