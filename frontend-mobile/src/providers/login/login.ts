...
export class LoginProvider {
  ...
  outputPayload = {
    ...
    description:'Un código de confirmación ha sido enviado a tu Email. Ingrésalo para crear tu cuenta exitosamente.'
  };

  private numberIntent:number;

  constructor(public http: HttpClient, private crypt: CypherProvider) {
  ...
  login( loginTO:LoginTO ){
    ...
    let URL = `${PATH_SECURITY.DOMAIN}/${PATH_SECURITY.LOGIN}`;
    return this.http.post( URL, loginTO ).timeout(15000).catch(this.handleError);
  }
  ...
  getParameter( paramter:string ) {
    let URL = `${PATH_SECURITY.DOMAIN}/${PATH_SECURITY.PARAMETER}?name=${paramter}`;
  ...
  createAccount( newAccount:NewAccountTO ) {
    return this.http.post(`${PATH_USER.DOMAIN}/${PATH_USER.CREATE}`,newAccount ).catch(this.handleError);
  }
  // 

  confirmCode( code:NewAccountTO ) {
    return this.http.post(`${PATH_USER.DOMAIN}/${PATH_USER.CODE}`,{code:code.code,user:code.user} ).catch( this.handleError );
  }
// 
...
// 
  ...
  getUserResetPass(body:resetPasswordTO){
    return this.http.post(`${PATH_SECURITY.DOMAIN}/${PATH_USER.GET_USER}`,body).catch(this.handleError);
    }
