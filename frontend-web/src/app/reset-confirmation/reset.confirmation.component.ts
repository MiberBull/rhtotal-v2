...
export class ResetConfirmationComponent {
  ...
  constructor(
    ...
    this.routerSubscription = this._router.queryParams.subscribe(
      (response) => {
        ...
        if (!('token' in this.queryParams) || !('user' in this.queryParams)) {
          this.isPetitionInvalid = true;
        }

        if (this.queryParams.token === '' || this.queryParams.user === '')
          this.isPetitionInvalid = true;
      },
      (error) => {
    ...
    this.myForm = this._formBuilder.group(
      {
        passwordFormControlName: [
          ...
          [
            ...
            Validators.pattern(EXPRESSION.PASSWORD),
          ],
        ],
  ...
  submit = (event) => {
    ...
    const newPasswordEncrypted = CryptoJS.AES.encrypt(
      values.passwordFormControlName,
      SECRETS.AES_PASSWORD_SECRET
    ).toString();

    const newPasswordConfirmedEncrypted = CryptoJS.AES.encrypt(
      values.passwordConfirmedFormControlName,
      SECRETS.AES_PASSWORD_SECRET
    ).toString();

    this.resetSubscription = this._resetService
      .sendResetPassword(
      ...
      .subscribe(
        ...
        (error) => {
          ...
          this.isAnError = true;
          this._dataService.setGeneralNotificationMessage(
            'Ocurrió un error, por favor vuelve a intentar.'
          );
